...
![Vue 双向绑定流程图](./images/two-way.png "Vue 双向绑定流程图")  
主要流程：
1. 通过 Observe 来监听 data 的属性，这里就是 Object.defineProperty
2. 当数据改变时，通过 set 来通知变化（通过 Dep 对象）。
3. Dep 对象则用于存储 Watcher（订阅者）的业务，通知 Watcher 数据的变化。
4. Watcher 订阅者，添加业务逻辑到 Dep 上，以便 Dep 来通知他。这里主要用于处理 DOM 逻辑。通过 update 来触发页面变化。
